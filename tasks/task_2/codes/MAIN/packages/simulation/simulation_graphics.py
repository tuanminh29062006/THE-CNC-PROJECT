...
def graphics(self):
    ...
    self.g0.set_data(self.travel_x, self.travel_y)
    self.canvas.draw_idle()
    self.log_bold(f"X:{self.append_x: 5.5f} | Y:{self.append_y: 5.5f} | Z:{self.append_z: 5.5f}\n")
    step_x = self.matlab_x[-1] if self.matlab_x else 0
    step_y = self.matlab_y[-1] if self.matlab_y else 0
    step_z = self.matlab_z[-1] if self.matlab_z else 0
    self.log_bold(f"Step X:{step_x: 5.5f} | Step Y:{step_y: 5.5f} | Step Z:{step_z: 5.5f}\n\n")
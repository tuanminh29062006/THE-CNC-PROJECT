...
def start(self):
    try:
        ...
        if self.resolution_time <= 0: raise ValueError
    except ValueError:
        self.log("Error - Resolution time must be a positive \nnumber")
        messagebox.showerror("Error", "Resolution time must be a positive number")
    
    self.is_running = True # Điều kiện cần
    self.interpret_parse()
    if not self.motions: # Điều kiện đủ
        self.is_running = False
        self.log("Nothing to run\n")
        return
    ...
    self.log_erase()
    self.log_bold("[SIM]")
    self.log_bold(" Start Simulation\n")
    
    self.simulation_line(0, 0.0, 0.0, 0.0, 1200)